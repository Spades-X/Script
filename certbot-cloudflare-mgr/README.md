...
一台运行 Ubuntu/Debian（或其他支持 Snap 的 Linux）的服务器。
2. 一键运行
你可以直接在服务器上运行以下命令来下载并启动脚本：
...
查看相关文件路径：展示所有配置文件的位置，并直接生成适用于 Mac/本地终端的 scp 下载命令。
📂 证书下载 (Mac/本地)
脚本会自动将证书同步到 /root/certs_download/。你可以通过以下命令将证书一键下载到 Mac 桌面：

*.bash
Shell
scp -r root@你的服务器IP:/root/certs_download/ ~/Desktop/
🛠 脚本结构
/root/.secrets/certbot/cloudflare.ini: 存储你的 Cloudflare API Token。
/usr/local/bin/copy_certs.sh: 核心同步逻辑脚本。
/etc/letsencrypt/renewal-hooks/deploy/: 存放自动续订触发钩子。
/root/certs_download/: 最终供下载的证书存放处。
⚠️ 安全提示
本脚本会将 Cloudflare API Token 存储在 /root/.secrets/ 目录下，并设置 600 权限。请确保不要将该目录暴露给非 root 用户。
建议在 GitHub 仓库中不要包含任何私人的 cloudflare.ini 文件。
📄 开源协议
MIT License
